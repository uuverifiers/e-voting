...
function delete(S : PreferenceList, el : Candidate) : (S_new : PreferenceList)
    requires multiset(S)[el] <= 1
    ensures forall s :: s in S_new ==> s != el
    ensures el in S ==> |S_new| == |S|-1
    ensures el !in S ==> S_new == S 

    ensures multiset(S_new)<= multiset(S) 
    ensures |S| != 0 && multiset(S)[el] == 1==> multiset(S_new) + multiset([el]) == multiset(S) 
    ensures forall i, k :: 0 <= i < |S| && S[i] == el && 0 <= k < i ==> S[k] == S_new[k] 
    ensures forall i, k :: 0 <= i < |S| && S[i] == el && i < k < |S| ==> S[k] == S_new[k-1] 
    /*
        ...
        Input:
            ...
            el (Candidate) : The element to remove
        Output:
...
method elim(V: Votes_PreferenceList, l: Candidate, C: Candidates) returns (V_new: Votes_PreferenceList)
