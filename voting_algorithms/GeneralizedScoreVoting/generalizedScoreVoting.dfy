type Vote_Score = map<nat,int>
type Votes_Score = seq<Vote_Score>
...
method CalculateElectionResult_APPROVAL_Internal(candidates: set<nat>, votes: Votes_Score) returns (winners:set<nat>, scorePerCandidate:map<nat,int>)
...
method CalculateElectionResult_APPROVAL(candidates: set<nat>, votes: Votes_Score) returns (winners:set<nat>, scorePerCandidate:map<nat,int>)
{
...
method CalculateElectionResult_Majority_Internal(candidates: set<nat>, votes: Votes_Score) returns (winners:set<nat>, scorePerCandidate:map<nat,int>)
  requires 0 !in candidates
  requires candidates!={}
  // all votes are for existing candidates and the given score is in allowedScores {0, 1}
  requires forall i :: (0<=i<|votes|) ==> (votes[i].Keys <= candidates && votes[i].Values <= {0, 1})
...
method CalculateElectionResult_Majority(candidates: set<nat>, votes: Votes_Score) returns (winners:set<nat>, scorePerCandidate:map<nat,int>)
{
  RuntimeRequires(0 !in candidates);
  RuntimeRequires(candidates!={});
  RuntimeRequires(forall i :: (0<=i<|votes|) ==> (votes[i].Keys <= candidates && votes[i].Values <= {0, 1}));
...
method CalculateElectionResult_COMBINED_APPROVAL_Internal(candidates: set<nat>, votes: Votes_Score) returns (winners:set<nat>, scorePerCandidate:map<nat,int>)
...
method CalculateElectionResult_COMBINED_APPROVAL(candidates: set<nat>, votes: Votes_Score) returns (winners:set<nat>, scorePerCandidate:map<nat,int>)
{
  RuntimeRequires(0 !in candidates);
  RuntimeRequires(candidates!={});
...
method CalculateElectionResult_SCORE_Internal(candidates: set<nat>, votes: Votes_Score) returns (winners:set<nat>, scorePerCandidate:map<nat,int>)
  requires 0 !in candidates
  requires candidates!={}
...
method CalculateElectionResult_SCORE(candidates: set<nat>, votes: Votes_Score) returns (winners:set<nat>, scorePerCandidate:map<nat,int>)
{
  RuntimeRequires(0 !in candidates);
  RuntimeRequires(candidates!={});
...
method CalculateElectionResult_SCORED_GENERAL(candidates: set<nat>, votes: Votes_Score, allowedScores: set<int>) returns (winners: set<nat>, scorePerCandidate:map<nat,int>)
  requires 0 !in candidates
...
ghost function scoreForCandidateFromSingleVote_SCORED_GENERAL(c:nat, vote:Vote_Score): (r:int)
...
ghost function scoreForSingleCandidate_SCORED_GENERAL(c:nat, votes:Votes_Score): (r:int)
ensures |votes|==0 ==> r==0
...
method getScorePerCandidate_SCORED_GENERAL(candidates: set<nat>, votes: Votes_Score) returns (scorePerCandidate:map<nat, int>)
...
ensures forall cand:nat :: (cand in candidates) ==> (scoreForSingleCandidate_SCORED_GENERAL(cand,votes) == scorePerCandidate[cand])
{
  ...
  {
    var singleVote:Vote_Score:= votes[i];
