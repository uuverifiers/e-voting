...
function relevantFactors(V: Votes_PreferenceList, F: FactorList, c: Candidate): (A0 : seq<real>)
    requires |V| == |F|
...
function calculateTotalValue(V : Votes_PreferenceList, C : Candidates, F : FactorList, c : Candidate) : (r : real)
    requires c in C
    requires forall f :: f in F ==> f >= 0.0
...
lemma incrementalSeq(V : Votes_PreferenceList, F : FactorList, i : nat)
    requires 0 <= i < |V| == |F|
...
lemma rankingValue(V : Votes_PreferenceList, C : Candidates, F : FactorList, c : Candidate)
    requires c in C
    requires forall f :: f in F ==> f >= 0.0
    requires forall v, c :: v in V && c in v ==> c in C 
    requires |V| == |F|
...
lemma RankingValue(V : Votes_PreferenceList, C : Candidates, F : FactorList, R : map<Candidate, real>)
    requires forall f :: f in F ==> f >= 0.0
    requires forall v, c :: v in V && c in v ==> c in C 
    requires |V| == |F|
    requires forall v :: v in V ==> |v| > 0
...
method CalculateTotalValue(V : Votes_PreferenceList, C : Candidates, F : FactorList) returns (R : map<Candidate, real>)
    requires forall f :: f in F ==> f >= 0.0
    requires forall v, c :: v in V && c in v ==> c in C 
    requires |V| == |F|
    requires forall v :: v in V ==> |v| > 0
